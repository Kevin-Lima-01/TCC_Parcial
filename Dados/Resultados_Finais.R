...
#  10. Instrumento tradeable-demand: carga, merge com BASE_M e diagnóstico
#  11. Aplicação 2SLS de kappa e psi com o instrumento tradeable-demand
#      (replica as colunas "Lagged unempl." vs. "Tradeable demand IV" da
#      Tabela I de Hazell et al. 2022)
#  12. Gráficos comparativos envolvendo o instrumento
#  13. Exportação consolidada de tabelas e gráficos (agora incluindo o IV)
###############################################################################
...
# Todas as bases de entrada usam o mesmo padrão: uma variável CAMINHO_*
# nomeada aqui no topo, referenciada mais abaixo no read.csv()/read_excel()
# correspondente -- nenhum caminho fica embutido direto dentro da chamada
# de leitura, para ficar tudo num único lugar fácil de editar.
PASTA_DADOS  <- "FGV/TCC/Dados"
PASTA_SAIDA  <- "FGV/TCC/Resultados"
if (!dir.exists(PASTA_SAIDA)) dir.create(PASTA_SAIDA, recursive = TRUE)

CAMINHO_DES_M       <- file.path(PASTA_DADOS, "Desemprego/DES_M.csv")
CAMINHO_INFLA_M     <- file.path(PASTA_DADOS, "Inflacao/infla_m.xlsx")
CAMINHO_INSTRUMENTO <- file.path(PASTA_DADOS, "IV/instrumento_tradable_demand_log.csv")
...
INFLA_M <- read_excel(CAMINHO_INFLA_M) %>%
...
construir_retas <- function(modelos, nomes, variaveis, dados_lista, var_y,
  ...
  for (k in seq_along(modelos)) {
    var_k      <- variaveis[k]
    y_k        <- var_y[k]
    # Usamos coef() (acessador nativo do fixest) em vez de tidy()+filter().
    # tidy() (via broom) pode rotular o coeficiente da variável endógena de
    # forma diferente em modelos de IV, mesmo quando etable() exibe o nome
    # original corretamente -- isso causava filter(term == var_k) retornar
    # zero linhas.
    #
    # Segunda armadilha, descoberta na prática: em modelos de IV, o PRÓPRIO
    # coef() nomeia o coeficiente da variável ENDÓGENA com o prefixo "fit_"
    # (ex.: "fit_taxa_desemprego"), mesmo que etable() mostre só
    # "taxa_desemprego". Em OLS não há esse prefixo. Por isso tentamos os
    # dois nomes antes de desistir.
    coefs_k   <- coef(modelos[[k]])
    nome_coef <- var_k
    if (!nome_coef %in% names(coefs_k)) {
      nome_alt <- paste0("fit_", var_k)
      if (nome_alt %in% names(coefs_k)) {
        nome_coef <- nome_alt
      } else {
        stop(sprintf(
          "construir_retas(): nem '%s' nem '%s' foram encontrados nos coeficientes do modelo %d ('%s'). Nomes disponíveis: %s",
          var_k, nome_alt, k, nomes[k], paste(names(coefs_k), collapse = ", ")
        ))
      }
    }
    estimativa <- unname(coefs_k[nome_coef])
    df_k       <- dados_lista[[k]]
    x_orig     <- df_k[[var_k]]
    y_obs      <- df_k[[y_k]]
    ...
    if (padronizar) {
      ...
      slope    <- estimativa * sd_x            # efeito por 1 DP do regressor
...
## ============================================================================
## 10. INSTRUMENTO TRADEABLE-DEMAND: CARGA, MERGE E DIAGNÓSTICO
## ============================================================================
# O instrumento foi construído em script separado (03_construir_instrumento.R):
# TradableDemand_it = soma_x [ S_bar_xi * Delta36m_log_S_{-i,x,t} ], com
# S_bar_xi restrito a setores tradable (agropecuária, indústria extrativa e
# de transformação) e renormalizado, e Delta36m_log_S_{-i,x,t} = crescimento
# em log de 36 meses do estoque de emprego formal do setor x no resto do
# Brasil (nacional menos o próprio município). Cobertura válida: ~2010-2024
# (precisa de 36 meses de histórico do estoque reconstruído, que por sua vez
# só existe a partir de 2007). Isso é MENOR que a amostra completa
# (2002-2026) usada nas Seções 1-9 -- a aplicação do instrumento portanto
# roda sobre uma subamostra, o que precisa ser reportado no texto.

# --- 10.1 Carregamento (caminho já definido no topo, Seção 0) --------------
instrumento <- read.csv(CAMINHO_INSTRUMENTO) %>%
  mutate(id_municipio = as.character(id_municipio))

# --- 10.2 Crosswalk id_municipio (IBGE) -> nome usado em BASE_M -------------
# ATENÇÃO: os nomes abaixo foram escritos para bater com o formato observado
# em BASE_M$municipio (ex.: "São Paulo (SP)"), incluindo o "Grande" em
# Vitória, que aparece assim na Tabela 1 do texto. Se algum nome estiver
# errado, o left_join do passo 10.3 descarta aquele município SEM avisar --
# por isso a checagem do passo 10.4 é obrigatória, não opcional.
crosswalk_municipio <- tribble(
  ...
  "3205309",     "Grande Vitória (ES)"
)

# Checagem OBRIGATÓRIA: todo nome do crosswalk deve existir em BASE_M, e
# todo município de BASE_M usado na amostra deve ter uma linha no crosswalk.
# Qualquer nome fora dessas duas listas é um erro de digitação a corrigir
# ANTES de prosseguir - não filtrar silenciosamente.
nomes_fora_de_base_m  <- setdiff(crosswalk_municipio$municipio, unique(BASE_M$municipio))
nomes_fora_do_crosswalk <- setdiff(unique(BASE_M$municipio), crosswalk_municipio$municipio)

cat("Nomes do crosswalk que NÃO aparecem em BASE_M$municipio (deveria ser 0):\n")
print(nomes_fora_de_base_m)
cat("\nNomes em BASE_M$municipio que NÃO têm crosswalk (ok ter outros, desde",
    "que os 16 da amostra do IV estejam cobertos):\n")
print(nomes_fora_do_crosswalk)

# --- 10.3 Construir a chave de data e mesclar com BASE_M -------------------
instrumento <- instrumento %>%
  left_join(crosswalk_municipio, by = "id_municipio") %>%
  mutate(data_date = ym(sprintf("%d-%02d", ano, mes))) %>%
  select(municipio, data_date, tradable_demand_log)

BASE_M <- BASE_M %>%
  left_join(instrumento, by = c("municipio", "data_date"))

# --- 10.4 Diagnóstico de cobertura -------------------------------------------
cat("\nCobertura do instrumento após o merge:\n")
BASE_M %>%
  filter(!is.na(tradable_demand_log)) %>%
  summarise(
    n_obs        = n(),
    n_municipios = n_distinct(municipio),
    data_min     = min(data_date),
    data_max     = max(data_date)
  ) %>%
  print()

# Quantas observações por município têm o instrumento? Município com
# cobertura muito abaixo dos demais merece nota à parte no texto.
BASE_M %>%
  filter(!is.na(tradable_demand_log)) %>%
  count(municipio, name = "n_obs_com_instrumento") %>%
  arrange(n_obs_com_instrumento) %>%
  print(n = 16)

# --- 10.5 Gráfico: instrumento vs. desemprego contemporâneo (São Paulo) ----
# Checagem visual de primeiro estágio antes de rodar qualquer 2SLS: o
# instrumento deveria comover (tipicamente negativamente) com o desemprego
# local, já que é uma proxy de choque de demanda por trabalho.
sp_instrumento <- BASE_M %>%
  filter(municipio == "São Paulo (SP)", !is.na(tradable_demand_log)) %>%
  select(data_date, taxa_desemprego, tradable_demand_log) %>%
  pivot_longer(c(taxa_desemprego, tradable_demand_log),
               names_to = "serie", values_to = "valor") %>%
  mutate(serie = recode(serie,
                        taxa_desemprego      = "Taxa de Desemprego (%)",
                        tradable_demand_log  = "Instrumento TradableDemand"))

fig_instrumento_vs_desemprego <- ggplot(sp_instrumento, aes(x = data_date, y = valor)) +
  geom_line(color = "#2c7fb8", linewidth = 0.7) +
  facet_wrap(~ serie, ncol = 1, scales = "free_y") +
  labs(
    title    = "São Paulo — Instrumento TradableDemand vs. Desemprego",
    subtitle = "Checagem visual de primeiro estágio antes do 2SLS",
    x = NULL, y = NULL
  ) +
  theme_minimal(base_size = 11) +
  theme(plot.title    = element_text(hjust = 0.5, face = "bold"),
...
print(fig_instrumento_vs_desemprego)


## ============================================================================
## 11. APLICAÇÃO 2SLS: KAPPA E PSI COM O INSTRUMENTO TRADEABLE-DEMAND
## ============================================================================
# Sintaxe de IV no fixest: y ~ controles_exogenos | FE | endogena ~ instrumento
#
# LEITURA DESTA SEÇÃO (papel de cada instrumento no TCC):
#   - "Lagged unempl. IV" (u_L12) é a IDENTIFICAÇÃO DE ROBUSTEZ PRINCIPAL
#     para psi e kappa nesta seção. É o análogo direto da coluna "(3) Lagged
#     unempl." de Hazell et al. (2022), passa no diagnóstico de primeiro
#     estágio (Seção 11.4) e reproduz o sinal e a ordem de grandeza da
#     estimativa em painel completo (Seções 4-5).
#   - "Tradeable demand IV" (tradable_demand_log) é apresentado como um
#     EXERCÍCIO EXPLORATÓRIO, com diagnóstico completo (primeiro estágio na
#     Seção 11.4; forma reduzida e teste de sobre-identificação na Seção
#     11.5; heterogeneidade por tipo de bem na Seção 11.6). O resultado --
#     primeiro estágio forte, mas sinal trocado no 2SLS e rejeição do teste
#     de sobre-identificação -- é reportado como um achado de robustez em
#     si (ver síntese na Seção 11.7), não como uma segunda estimativa
#     igualmente válida de psi/kappa. As duas fontes de variação NÃO são
#     combinadas num 2SLS final: o modelo com os dois instrumentos juntos
#     (Seção 11.5-b) existe apenas para RODAR o teste de sobre-
#     identificação, não para produzir uma estimativa de consenso --
#     misturar um instrumento inválido com um válido contamina o resultado
#     combinado em vez de diluir o problema.
#
# Para psi (eq. 19): a variável endógena é taxa_desemprego (nível corrente).
#   - "Lagged unempl. IV"     -> instrumento = u_L12 (desemprego defasado 12m)
#   - "Tradeable demand IV"   -> instrumento = tradable_demand_log
#
# Para kappa (eq. 17): as variáveis endógenas são soma_u e soma_phat (somas
# descontadas de desemprego e preço relativo futuros - ambas potencialmente
# endógenas porque dependem de realizações futuras correlacionadas com o
# erro). Instrumentamos as DUAS ao mesmo tempo:
#   - "Lagged unempl. IV"   -> soma_u ~ u_L12,               soma_phat ~ phat_N_L12
#   - "Tradeable demand IV" -> soma_u ~ tradable_demand_log, soma_phat ~ phat_N_L12
#     (troca-se apenas o instrumento do desemprego, mantendo o preço relativo
#     instrumentado por seu próprio lag - mesma lógica da coluna 4 da
#     Tabela I de Hazell et al., nota de rodapé 20 do paper)
#
# IMPORTANTE: todas as regressões desta seção rodam sobre a subamostra com
# instrumento disponível (~2010-2024), não a amostra completa (2002-2026)
# usada nas Seções 4-9. As colunas "Lagged unempl. IV" desta seção usam a
# MESMA subamostra (para que a comparação com "Tradeable demand IV" seja
# sobre observações idênticas, não apenas cobertura de instrumento diferente).

amostra_iv <- BASE_M %>% filter(!is.na(tradable_demand_log))

cat("\nTamanho da subamostra usada nas regressões de IV desta seção:",
    nrow(amostra_iv), "observações |", n_distinct(amostra_iv$municipio),
    "municípios (vs.", nrow(BASE_M), "na amostra completa)\n")

# --- 11.1 OLS na mesma subamostra (referência de comparação) ----------------
# Reestima psi_nt e kappa_nt (Seção 4) na subamostra restrita, para que a
# comparação com os IVs abaixo isole o efeito do método de identificação,
# não uma mudança de amostra.
psi_nt_ols_sub   <- feols(VNT_12m ~ taxa_desemprego + phat_N | municipio + data,
                          data = amostra_iv, cluster = ~municipio)
kappa_nt_ols_sub <- feols(VNT_12m ~ soma_u + soma_phat | municipio + data,
                          data = amostra_iv, cluster = ~municipio)

# --- 11.2 psi via 2SLS: OLS vs. Lagged unempl. IV vs. Tradeable demand IV --
psi_nt_iv_lag      <- feols(VNT_12m ~ phat_N | municipio + data | taxa_desemprego ~ u_L12,
                            data = amostra_iv, cluster = ~municipio)
psi_nt_iv_tradable <- feols(VNT_12m ~ phat_N | municipio + data | taxa_desemprego ~ tradable_demand_log,
                            data = amostra_iv, cluster = ~municipio)

etable(psi_nt_ols_sub, psi_nt_iv_lag, psi_nt_iv_tradable,
       headers = c("OLS", "Lagged unempl. IV (principal)", "Tradeable demand IV (exploratório)"),
       title   = "Tabela 12 — psi (Non-Tradable): OLS vs. IV",
       digits  = 4,
       notes   = "Mesma subamostra (~2010-2024) nas três colunas. Variável
                  endógena instrumentada: taxa_desemprego. Replica o contraste
                  das colunas (3) e (4) da Tabela I de Hazell et al. (2022).
                  'Lagged unempl. IV' é a identificação de robustez adotada
                  no texto; 'Tradeable demand IV' é reportado como exercício
                  exploratório -- ver diagnóstico completo na Seção 11.5.")

# --- 11.3 kappa via 2SLS: OLS vs. Lagged unempl. IV vs. Tradeable demand IV
kappa_nt_iv_lag      <- feols(VNT_12m ~ 1 | municipio + data | soma_u + soma_phat ~ u_L12 + phat_N_L12,
                              data = amostra_iv, cluster = ~municipio)
kappa_nt_iv_tradable <- feols(VNT_12m ~ 1 | municipio + data | soma_u + soma_phat ~ tradable_demand_log + phat_N_L12,
                              data = amostra_iv, cluster = ~municipio)

etable(kappa_nt_ols_sub, kappa_nt_iv_lag, kappa_nt_iv_tradable,
       headers = c("OLS", "Lagged unempl. IV (principal)", "Tradeable demand IV (exploratório)"),
       title   = "Tabela 13 — kappa (Non-Tradable): OLS vs. IV",
       digits  = 4,
       notes   = "Mesma subamostra (~2010-2024) nas três colunas. Variáveis
                  endógenas instrumentadas: soma_u e soma_phat (soma_phat
                  sempre instrumentada por phat_N_L12; só o instrumento de
                  soma_u muda entre as colunas 2 e 3). Mesma leitura da
                  Tabela 12: 'Lagged unempl. IV' é a robustez principal;
                  'Tradeable demand IV' é exploratório (Seção 11.5).")

# --- 11.4 Primeiro estágio (força do instrumento) -- MÉTODO CORRIGIDO ------
# A primeira versão desta seção rodava uma regressão auxiliar separada
# (taxa_desemprego ~ instrumento + controles) e chamava fitstat(modelo, "f").
# Isso está ERRADO para diagnóstico de instrumento fraco: fitstat(.,"f") num
# feols comum testa a significância CONJUNTA de TODOS os regressores daquela
# regressão (o instrumento E o controle exógeno phat_N juntos) - não isola a
# força do instrumento em si. Prova disso: o coeficiente de u_L12 no 1o
# estágio de psi tinha t = 0.3766/0.0798 = 4,72 (F = t² ≈ 22, instrumento
# forte), mas o fitstat(.,"f") daquele modelo reportou 0,65 - um número
# completamente diferente e enganoso.
#
# O jeito certo: usar fitstat() diretamente nos modelos de IV JÁ ESTIMADOS
# (11.2 e 11.3), pedindo o F-stat de primeiro estágio específico de cada
# variável endógena ("ivf1" para a 1a endógena, "ivf2" para a 2a, quando
# houver). Esse é o F-stat de Sanderson-Windmeijer / Staiger-Stock que o
# próprio fixest calcula internamente para o IV, e é o que deve ser citado
# no texto -- não o número da versão anterior desta seção.
cat("\nF-stat de primeiro estágio (regra de Staiger-Stock: >10 é forte):\n")

cat("\npsi, endógena = taxa_desemprego:\n")
cat("  Lagged unempl. IV:    ", fitstat(psi_nt_iv_lag, "ivf1")[[1]]$stat, "\n")
cat("  Tradeable demand IV:  ", fitstat(psi_nt_iv_tradable, "ivf1")[[1]]$stat, "\n")

cat("\nkappa, endógenas = soma_u (ivf1) e soma_phat (ivf2):\n")
cat("  Lagged unempl. IV:    ",
    fitstat(kappa_nt_iv_lag, "ivf1")[[1]]$stat, " (soma_u) | ",
    fitstat(kappa_nt_iv_lag, "ivf2")[[1]]$stat, " (soma_phat)\n")
cat("  Tradeable demand IV:  ",
    fitstat(kappa_nt_iv_tradable, "ivf1")[[1]]$stat, " (soma_u) | ",
    fitstat(kappa_nt_iv_tradable, "ivf2")[[1]]$stat, " (soma_phat)\n")

# Versão em tabela, para citar diretamente no texto/apêndice do TCC
tabela_forca_instrumento <- tibble::tribble(
  ~modelo,   ~instrumento,           ~endogena,    ~f_stat,
  "psi",     "Lagged unempl. IV",    "taxa_desemprego", fitstat(psi_nt_iv_lag, "ivf1")[[1]]$stat,
  "psi",     "Tradeable demand IV",  "taxa_desemprego", fitstat(psi_nt_iv_tradable, "ivf1")[[1]]$stat,
  "kappa",   "Lagged unempl. IV",    "soma_u",           fitstat(kappa_nt_iv_lag, "ivf1")[[1]]$stat,
  "kappa",   "Lagged unempl. IV",    "soma_phat",        fitstat(kappa_nt_iv_lag, "ivf2")[[1]]$stat,
  "kappa",   "Tradeable demand IV",  "soma_u",           fitstat(kappa_nt_iv_tradable, "ivf1")[[1]]$stat,
  "kappa",   "Tradeable demand IV",  "soma_phat",        fitstat(kappa_nt_iv_tradable, "ivf2")[[1]]$stat
) %>%
  mutate(instrumento_fraco = f_stat < 10)

print(tabela_forca_instrumento)


## ============================================================================
## 11.5 DIAGNÓSTICO DO SINAL TROCADO: FORMA REDUZIDA E TESTE DE SOBRE-IDENTIFICAÇÃO
## ============================================================================
# O primeiro estágio de tradable_demand_log está FORTE e com o sinal
# esperado (mais TradableDemand -> menos desemprego). Mesmo assim, o
# segundo estágio de psi/kappa (Seções 11.2-11.3) sai com sinal trocado em
# relação à teoria e ao OLS. Como o 2SLS com 1 instrumento é uma razão:
#
#   beta_2SLS = (efeito do instrumento sobre a inflação, direto)
#               -------------------------------------------------
#               (efeito do instrumento sobre o desemprego, 1o estágio)
#
# um numerador negativo (a forma reduzida) combinado com denominador
# negativo (1o estágio, já confirmado) produz um beta positivo. Isso é
# consistente com uma violação de exclusão: o instrumento pode afetar a
# inflação NT por um canal que não passa pelo desemprego local. Esse
# diagnóstico é feito ANTES de olhar a heterogeneidade por tipo de bem
# (Seção 11.6) porque é ele que motiva a hipótese testada lá.

# --- (a) Forma reduzida: efeito DIRETO do instrumento sobre a inflação ----
# Não instrumenta nada -- é só a correlação (com FE) entre o instrumento e
# cada variável dependente. Se sair negativa e significativa para NT, o
# quebra-cabeça descrito acima está confirmado nos dados.
forma_reduzida_nt    <- feols(VNT_12m   ~ tradable_demand_log + phat_N | municipio + data,
                              data = amostra_iv, cluster = ~municipio)
forma_reduzida_ipca  <- feols(VIPCA_12m ~ tradable_demand_log          | municipio + data,
                              data = amostra_iv, cluster = ~municipio)
forma_reduzida_t     <- feols(VT_12m    ~ tradable_demand_log          | municipio + data,
                              data = amostra_iv, cluster = ~municipio)

etable(forma_reduzida_nt, forma_reduzida_ipca, forma_reduzida_t,
       headers = c("Non-Tradable", "IPCA Total", "Tradable"),
       title   = "Tabela 16 — Forma Reduzida: Efeito Direto do Instrumento sobre a Inflação",
       digits  = 4,
       notes   = "Regressão direta (sem instrumentar nada) de cada medida de inflação
                  sobre tradable_demand_log. Um coeficiente negativo aqui é o que produz
                  o sinal trocado no 2SLS, dado que o 1o estágio já é negativo
                  (mais TradableDemand -> menos desemprego). Padrão de magnitude
                  observado (|Tradable| > |IPCA| > |NT|, com NT não significativo)
                  é o OPOSTO do que um choque de demanda local por trabalho
                  preveria (que deveria concentrar o efeito em NT e ser
                  próximo de zero em Tradable, precificado nacionalmente) --
                  ver interpretação ao final desta seção.")

# --- (b) Teste de sobre-identificação: os dois instrumentos concordam? ----
# Usa u_L12 E tradable_demand_log juntos para instrumentar taxa_desemprego
# (sistema sobre-identificado: 2 instrumentos para 1 endógena). O teste de
# Sargan/Hansen testa se as restrições de sobre-identificação são válidas
# -- ou seja, se os dois instrumentos produzem estimativas consistentes
# entre si. Rejeição (p<0.05) é evidência formal de que pelo menos um dos
# dois viola a restrição de exclusão. Dado que u_L12 sozinho dá sinal
# correto e tradable_demand_log sozinho dá sinal trocado, uma rejeição
# aponta o dedo para tradable_demand_log como o problemático.
#
# IMPORTANTE: este modelo com os dois instrumentos juntos serve SOMENTE
# para rodar o teste de sobre-identificação abaixo. Ele NÃO deve ser
# reportado como a estimativa "final" de psi -- misturar um instrumento
# inválido com um válido não dilui o problema, apenas o espalha para o
# coeficiente combinado (repare que, na Tabela 17, a coluna "Os dois
# juntos" já perde magnitude e significância em relação à coluna "Só
# Lagged unempl."). A estimativa de robustez principal do texto continua
# sendo a coluna "Lagged unempl. IV" das Tabelas 12 e 13.
psi_nt_iv_sobreidentificado <- feols(
  VNT_12m ~ phat_N | municipio + data | taxa_desemprego ~ u_L12 + tradable_demand_log,
  data = amostra_iv, cluster = ~municipio
)

etable(psi_nt_iv_lag, psi_nt_iv_tradable, psi_nt_iv_sobreidentificado,
       headers = c("Só Lagged unempl.", "Só Tradeable demand", "Os dois juntos (só p/ teste Sargan)"),
       title   = "Tabela 17 — psi: Instrumentos Separados vs. Sobre-identificado",
       digits  = 4,
       notes   = "A coluna 'Os dois juntos' usa u_L12 e tradable_demand_log
                  simultaneamente como instrumentos de taxa_desemprego e existe
                  apenas para viabilizar o teste de Sargan abaixo -- não é uma
                  estimativa de consenso a ser citada como resultado principal.")

cat("\nTeste de sobre-identificação (Sargan/Hansen) para psi:\n")
print(fitstat(psi_nt_iv_sobreidentificado, "sargan"))
cat("\nInterpretação: p < 0.05 rejeita a hipótese de que os dois instrumentos",
    "são igualmente válidos -- evidência formal de violação de exclusão em",
    "pelo menos um deles (aqui, candidato natural: tradable_demand_log,",
    "já que é o único dos dois que produz sinal trocado quando usado sozinho).\n")


## ============================================================================
## 11.6 HETEROGENEIDADE POR TIPO DE BEM: TESTANDO O CANAL DE COMMODITIES
## ============================================================================
# A forma reduzida (11.5) sugere que tradable_demand_log pode estar
# carregando um choque de custo/preço de commodities que passa direto para
# os preços ao consumidor -- inclusive tradable, via importação/insumos --
# e não um choque puro de demanda por trabalho local. Isso faz sentido: o
# instrumento é construído a partir do crescimento de setores como
# agropecuária e indústria extrativa (Seção 10), precisamente os mais
# expostos a ciclos de preços internacionais de commodities.
#
# Se essa hipótese estiver certa, o efeito de tradable_demand_log via 2SLS
# NÃO deveria ser menor em Tradable do que em Non-Tradable (ao contrário da
# predição de choque de demanda local, testada originalmente nesta seção).
# As duas tabelas abaixo mostram exatamente essa comparação para psi e kappa.

psi_ipca_iv_tradable <- feols(VIPCA_12m ~ 1 | municipio + data | taxa_desemprego ~ tradable_demand_log,
                              data = amostra_iv, cluster = ~municipio)
psi_t_iv_tradable    <- feols(VT_12m    ~ 1 | municipio + data | taxa_desemprego ~ tradable_demand_log,
                              data = amostra_iv, cluster = ~municipio)
kappa_ipca_iv_tradable <- feols(VIPCA_12m ~ 1 | municipio + data | soma_u ~ tradable_demand_log,
                                data = amostra_iv, cluster = ~municipio)
kappa_t_iv_tradable    <- feols(VT_12m    ~ 1 | municipio + data | soma_u ~ tradable_demand_log,
                                data = amostra_iv, cluster = ~municipio)

etable(psi_nt_iv_tradable, psi_ipca_iv_tradable, psi_t_iv_tradable,
       headers = c("Non-Tradable", "IPCA Total", "Tradable"),
       title   = "Tabela 14 — psi via Tradeable Demand IV, por Tipo de Bem",
       digits  = 4,
       notes   = "Sob um choque de demanda local por trabalho, o coeficiente em
                  Tradable deveria ser o mais próximo de zero dos três (bem
                  precificado nacionalmente, Seção III.B do paper). Combinado
                  com a Tabela 16, um efeito que NÃO se concentra em Non-Tradable
                  é evidência a favor do canal de custo de commodities, não de
                  uma falha isolada da regressão.")

etable(kappa_nt_iv_tradable, kappa_ipca_iv_tradable, kappa_t_iv_tradable,
       headers = c("Non-Tradable", "IPCA Total", "Tradable"),
       title   = "Tabela 15 — kappa via Tradeable Demand IV, por Tipo de Bem",
       digits  = 4,
       notes   = "soma_phat omitida nas colunas IPCA/Tradable (o mecanismo de
                  reversão à média do preço relativo NT não se aplica a essas
                  variáveis dependentes) - mesma convenção da Tabela 1 do texto.
                  Mesma leitura da Tabela 14 quanto ao canal de commodities.")


## ============================================================================
## 11.7 SÍNTESE PARA O TEXTO DO TCC
## ============================================================================
# Resumo do que a Seção 11 estabelece, para orientar a redação:
#
#   1. "Lagged unempl. IV" é a identificação de robustez PRINCIPAL para psi
#      (Tabela 12) -- sinal e ordem de grandeza coerentes com o painel
#      completo (Seções 4-5) e primeiro estágio forte (F ~ 238, Seção 11.4).
#
#   2. "Tradeable demand IV" é apresentado como exercício exploratório, com
#      diagnóstico completo: primeiro estágio forte (F entre ~30 e ~56,
#      Seção 11.4), mas (i) sinal trocado no 2SLS frente à teoria e ao OLS,
#      (ii) forma reduzida com efeito direto do instrumento sobre a inflação
#      MAIOR em Tradable do que em Non-Tradable -- o oposto do previsto por
#      um canal de demanda local (Tabela 16) --, e (iii) rejeição do teste
#      de sobre-identificação ao combinar os dois instrumentos (Tabela 17,
#      Sargan p < 0.001). Em conjunto, a evidência aponta para uma violação
#      de exclusão: o instrumento provavelmente carrega um choque de preço/
#      custo de commodities (coerente com sua composição setorial -- Seção
#      10 -- concentrada em agropecuária e indústria extrativa) que afeta a
#      inflação por um canal que não passa pelo desemprego local.
#
#   3. Os dois instrumentos NÃO são combinados num 2SLS final -- o modelo
#      combinado (Seção 11.5-b) serve apenas para o teste de Sargan.
#
#   4. Esse resultado não invalida a réplica: ele revela uma diferença
#      estrutural entre a economia americana do paper original (50 estados
#      diversificados) e a brasileira (16 metrópoles, com setores tradable
#      mais concentrados em commodities sujeitas a choques de preço
#      nacional/internacional que vazam inclusive para os non-tradables) --
#      um achado genuíno sobre limites de identificação nesse contexto,
#      não uma falha da estratégia empírica.


## ============================================================================
## 12. GRÁFICOS COMPARATIVOS ENVOLVENDO O INSTRUMENTO
## ============================================================================
# Reaproveita construir_retas()/grafico_retas() (Seção 7) para comparar
# visualmente a inclinação de psi (Non-Tradable) sob os três métodos de
# identificação: OLS, Lagged unempl. IV e Tradeable demand IV.

retas_iv_psi <- construir_retas(
  modelos     = list(psi_nt_ols_sub, psi_nt_iv_lag, psi_nt_iv_tradable),
  nomes       = c("OLS", "Lagged unempl. IV", "Tradeable demand IV"),
  variaveis   = rep("taxa_desemprego", 3),
  dados_lista = list(amostra_iv, amostra_iv, amostra_iv),
  var_y       = "VNT_12m",
  padronizar  = FALSE
)

fig_comp_iv_psi <- grafico_retas(
  retas_iv_psi,
  paleta    = c("OLS" = "#3B4252", "Lagged unempl. IV" = "#D08770",
                "Tradeable demand IV" = "#88C0D0"),
  titulo    = "psi (Non-Tradable) — OLS vs. Lagged unempl. IV vs. Tradeable demand IV",
  subtitulo = "Mesma subamostra (~2010-2024). Lagged unempl. IV = robustez principal;\nTradeable demand IV = exploratório, falha no teste de sobre-identificação (Seção 11.6).",
  x_lab     = "Taxa de desemprego (%)",
  y_lab     = "Inflação Non-Tradable acumulada 12m (%)"
)

print(fig_comp_iv_psi)


## ============================================================================
## 13. EXPORTAÇÃO CONSOLIDADA
## ============================================================================
# Reúne todas as tabelas de regressão em um único arquivo de texto (mais
# fácil de revisar/colar no TCC do que abrir os outputs separados no
...
etable(psi_nt_ols_sub, psi_nt_iv_lag, psi_nt_iv_tradable,
       headers = c("OLS", "Lagged unempl. IV (principal)", "Tradeable demand IV (exploratório)"),
       title = "Tabela 12 — psi (Non-Tradable): OLS vs. IV", digits = 4)

etable(kappa_nt_ols_sub, kappa_nt_iv_lag, kappa_nt_iv_tradable,
       headers = c("OLS", "Lagged unempl. IV (principal)", "Tradeable demand IV (exploratório)"),
       title = "Tabela 13 — kappa (Non-Tradable): OLS vs. IV", digits = 4)

cat("\nF-stat de primeiro estágio (Staiger-Stock, citar no texto):\n")
print(tabela_forca_instrumento)

etable(forma_reduzida_nt, forma_reduzida_ipca, forma_reduzida_t,
       headers = c("Non-Tradable", "IPCA Total", "Tradable"),
       title = "Tabela 16 — Forma reduzida: efeito direto do instrumento", digits = 4)

etable(psi_nt_iv_lag, psi_nt_iv_tradable, psi_nt_iv_sobreidentificado,
       headers = c("Só Lagged unempl.", "Só Tradeable demand", "Os dois juntos (só p/ teste Sargan)"),
       title = "Tabela 17 — psi: instrumentos separados vs. sobre-identificado", digits = 4)
...
etable(psi_nt_iv_tradable, psi_ipca_iv_tradable, psi_t_iv_tradable,
       headers = c("Non-Tradable", "IPCA Total", "Tradable"),
       title = "Tabela 14 — psi via Tradeable Demand IV, por tipo de bem", digits = 4)

etable(kappa_nt_iv_tradable, kappa_ipca_iv_tradable, kappa_t_iv_tradable,
       headers = c("Non-Tradable", "IPCA Total", "Tradable"),
       title = "Tabela 15 — kappa via Tradeable Demand IV, por tipo de bem", digits = 4)
