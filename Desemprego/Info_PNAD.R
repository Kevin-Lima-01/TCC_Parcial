...
####
# Taxas
PNADB <- read_excel("C:/Users/KEVIN/Desktop/Lições FGV/TCC/Dados/Desemprego/PNAD_INFO/PNAD_INFO.xlsx")
...
PNAD <- PNADB %>%
  pivot_longer(cols = -municipio,
               names_to = "data",
               values_to = "taxa_informalidade") %>%
  mutate(
    taxa_informalidade = as.numeric(taxa_informalidade),
    data = ym(data)
  ) %>%
...
interpolar_periodo <- function(df_periodo, metodo, nome_periodo) {
  if(nrow(df_periodo) < 2) {
    return(
      data.frame(
        ...
        taxa_informalidade = df_periodo$taxa_informalidade[1]
      )
    )
  }
  df_periodo <- df_periodo %>% filter(!is.na(taxa_informalidade))
  if(nrow(df_periodo) < 2) {
    return(NULL)
  }
  ...
  y_orig <- df_periodo$taxa_informalidade
  if(metodo == "linear") {
    df_mensal$taxa_informalidade <- approx(
      x = x_orig, 
      y = y_orig,
    ...
    if(length(x_orig) >= 3) {
      ...
      df_mensal$taxa_informalidade <- spline_fit(df_mensal$data_num)
    } else {
      df_mensal$taxa_informalidade <- approx(
        x = x_orig, 
        y = y_orig,
  ...
  df_mensal$taxa_informalidade <- pmax(0, pmin(100, df_mensal$taxa_informalidade))
  for(i in 1:nrow(df_periodo)) {
    mes_obs <- df_periodo$data_date[i]
    idx <- which(df_mensal$data == mes_obs)
    if(length(idx) > 0) {
      df_mensal$taxa_informalidade[idx] <- df_periodo$taxa_informalidade[i]
    }
  }
...
ggplot(sp_final, aes(x = data, y = taxa_informalidade)) +
  geom_line(data = sp_final %>% filter(periodo == "Pré-Pandemia", !is.na(taxa_informalidade)),
            aes(color = "Pré-Pandemia"), size = 1) +
  geom_line(data = sp_final %>% filter(periodo == "Pós-Pandemia", !is.na(taxa_informalidade)),
            aes(color = "Pós-Pandemia"), size = 1) +
  geom_point(data = sp_final %>% filter(!is.na(taxa_informalidade)),
  ...
  annotate("text",
           ...
           y = max(sp_final$taxa_informalidade, na.rm = TRUE) * 0.9,
...
base_mensal <- PNAD_mensal_completo %>%
  select(municipio, data, taxa_informalidade) %>%
  mutate(
...
# Exportar
output_dir <- "C:/Users/KEVIN/Desktop/Lições FGV/TCC/Dados/Desemprego/PNAD_INFO/"
write_xlsx(base_trimestral, paste0(output_dir, "PNADI_t.xlsx"))
write_xlsx(base_mensal, paste0(output_dir, "PNADI_m.xlsx"))
